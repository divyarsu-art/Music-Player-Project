...
function openPlayer(title, artist, color, imagePath, lyrics = "No lyrics available") {
  ...
  if (gradient) {
    // Hide the img, show a gradient div instead
    albumBox.style.display = "none";

    let gradientDiv = document.getElementById("albumGradientBox");
    if (!gradientDiv) {
      gradientDiv = document.createElement("div");
      gradientDiv.id = "albumGradientBox";
      albumBox.parentNode.insertBefore(gradientDiv, albumBox.nextSibling);
    }
    ...
    albumBox.src = imagePath;

    const gradientDiv = document.getElementById("albumGradientBox");
...
function openPlaylist(name) {
  ...
  if (!playlist.songs || playlist.songs.length === 0) {
    ...
    playlist.songs.forEach((song, index) => {
      ...
      div.onclick = () =>
  openPlayer(song.title, song.artist, "#ff4da6", null, "No lyrics available", playlist.gradient);
